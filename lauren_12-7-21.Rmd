...
# Outcome: Number of Arrests$_{county, month}$

```{r, echo = TRUE}

dui.model = feols(DUI_Total ~ i(time_to_treat, treat, ref = 0) | ## Model
                      fips_county_code + fd, ## Fixed Effects
                    cluster = ~fips_county_code + fd, ## Se Clustering
                    data = check7)
...
# Outcome: Proportion Black DUI Arrests$_{county, month}$

```{r, echo = TRUE}


p.black.model = feols(DUI_p.Black ~ i(time_to_treat, treat, ref = 0) | ## Model
                      fips_county_code + fd, ## Fixed Effects
                    cluster = ~fips_county_code + fd, ## Se clustering
                    data = check7)
